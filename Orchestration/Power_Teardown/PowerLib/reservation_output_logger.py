...
def add_screen_to_logger(logger, api_session, reservation_id):
    for handler in logger.handlers:
        if isinstance(handler, ReservationOutputHandler):
            ...
            return logger
        # ReservationOutputHandler is in logger.handlers.
    # if len(logger.handlers) > 1:  # If we've already added our loggers
    #     return logger
    # logger.level = DEBUG
    ...
    web_reservation_output_handler.setFormatter(logging.Formatter('Power Service: %(message)s'))
...
def get_reservation_output_logger(context):
    ...
    return add_screen_to_logger(logger, CloudShellSessionContext(context).get_api(), context.reservation.reservation_id)
